...
export const obtenerInventarioActions = (af_codigo_generico: string, altaS_corr: number, estabL_CORR: number) => async (dispatch: Dispatch, getState: any): Promise<boolean> => {
  ...
  if (token) {
    ...
    try {
      const res = await axios.get(`${import.meta.env.VITE_CSRF_API_URL}/TraeInvxID?af_codigo_generico=${af_codigo_generico}&altaS_corr=${altaS_corr}&estabL_CORR=${estabL_CORR}`, config);
