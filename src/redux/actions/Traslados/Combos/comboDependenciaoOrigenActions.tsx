...
export const comboDependenciaOrigenActions = (serCorr: string) => async (dispatch: Dispatch, getState: any) => {
  ...
  if (token) {
    ...
    dispatch({ type: COMBO_DEPENDENCIA_ORIGEN_REQUEST });
    ...
    try {
      ...
      if (res.status === 200) {
        dispatch({
          type: COMBO_DEPENDENCIA_ORIGEN_SUCCESS,
          payload: res.data,
        });
      } else {
        dispatch({ type: COMBO_DEPENDENCIA_ORIGEN_FAIL });
      }
    } catch (err) {
