...
export const listaAltasModificarActions = (fDesde: string, fHasta: string, af_codigo_generico: string, altasCorr: number, idocumento: number, establ_corr: number) => async (dispatch: Dispatch): Promise<boolean> => {

  dispatch({ type: OBTENER_ALTAS_MODIFICAR_REQUEST });
  ...
  try {
    ...
    if (res.status === 200) {
      if (res.data?.length) {
        dispatch({
          type: OBTENER_ALTAS_MODIFICAR_SUCCESS,
          payload: res.data,
        });
        return true;
      } else {
        dispatch({
          type: OBTENER_ALTAS_MODIFICAR_FAIL,
